...
location strategy_add(const Matrix<double, m, n> &A, const Matrix<double, m, n> &B) {
  ...
  if (location::ram == A.loc)
    t_fpga += m*n * ram2fpga_speed;
  else
    t_ram  += m*n * fpga2ram_speed;
  ...
  t_ram  += m*n * soft_dot_speed;
  t_fpga += m*n * fpga_speed + fpga_dot_lat;
 ...
 */
template <size_t m, size_t n>
void matrix_add_engine(Matrix<double, m, n> &A,
                       Matrix<double, m, n> &B,
  ...
  if (location::fpga == loc){
    ...
    if (A.tr)
      fpgaMtrxTrn(&MTRXD1, m, n,)
    fpgaMtrxAdd(&MTRXD1, m, n, A.fpga_idx, B.fpga_idx, C.fpga_idx);
  }
  else {
